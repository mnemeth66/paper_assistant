...
def render_title_and_author(paper_entry: dict, idx: int) -> str:
    ...
    authors = [a["name"] for a in paper_entry.get("authors", [])]

    # Extract arXiv ID to construct link
    arxiv_id = (
        paper_entry.get("ARXIVID")
        or paper_entry.get("externalIds", {}).get("ArXiv")
        or "unknown"
    )
    arxiv_url = f"https://arxiv.org/abs/{arxiv_id}"

    paper_string = f"{idx}. [{title}](#user-content-link{idx})\n"
    paper_string += f'**Authors:** {", ".join(authors)} ([link]({arxiv_url}))\n'
