...
def render_paper(paper_entry: dict, idx: int) -> str:
    ...
    :return: a markdown formatted string showing the DOI, title, arxiv url, abstract, authors, score and comment (if those fields exist)
    """
    # get the DOI
    doi = paper_entry["doi"]
    # get the title
...
def render_title_and_author(paper_entry: dict, idx: int) -> str:
    ...
    authors = paper_entry["authors"]
    doi = paper_entry["doi"]
    # Github Pages prepends the links created in render_paper with 
    # user-content- for some reason - add them to the links to fix that.
    # This will likely depend on who is hosting the webpage.
    paper_string = f"{idx}. [{title}](#user-content-link{idx})\n" 
    paper_string += f'**Authors:** {", ".join(authors)}\n'
    biorxiv_url = f"https://www.biorxiv.org/content/{doi}"
    paper_string += f'[link]({biorxiv_url})\n'
    return paper_string
...
def render_md_string(papers_dict):
    ...
    for key, paper in papers_dict.items():
        ...
        if score >= 6:  # Apply score threshold here, eventually make this a toggle [TODO]
            paper['SCORE'] = score
